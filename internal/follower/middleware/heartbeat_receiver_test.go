package middleware

import (
	"fmt"
	"net"
	"testing"
	"time"
)

func startTestServer(t *testing.T, address string, stop chan struct{}) {
	fmt.Println("ADDRESS " + address)
	err := expose(address, stop)
	if err != nil {
		t.Errorf("Server failed : %v", err)
	}
}

func sendTestUDPPacket(t *testing.T, url string, stop chan struct{}) {
	fmt.Println("URL DIAL " + url)
	conn, err := net.Dial("udp", url)
	if err != nil {
		t.Fatalf("Failed to dial server: %v", err)
	}

	message := []byte("test message")
	_, err = conn.Write(message)
	if err != nil {
		t.Fatalf("Failed to send packet: %v", err)
	}
	conn.Close()

	time.Sleep(100 * time.Millisecond)
	close(stop)
}

func TestExpose(t *testing.T) {
	stop := make(chan struct{})
	localAddress := "127.0.0.1:9003"

	go startTestServer(t, localAddress, stop)
	time.Sleep(100 * time.Millisecond)

	sendTestUDPPacket(t, localAddress, stop)
}

func TestSubscribeAndExpose(t *testing.T) {
	audioPort := "9000"
	heartbeatPort := "9001"
	stop := make(chan struct{})
	ipProvider := MockIPProvider{FakeIP: []byte{127, 0, 0, 1}}
	server := StartMockHTTPServer(t)
	endpointProvider := MockEndpointProvider{server.URL}

	go SubscribeAndExpose(heartbeatPort, audioPort, stop, ipProvider, endpointProvider)
	time.Sleep(100 * time.Millisecond)

	address := ipProvider.GetIP().String() + ":" + heartbeatPort
	sendTestUDPPacket(t, address, stop)
}
